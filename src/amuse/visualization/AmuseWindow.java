...
import amuse.visualization.amuseAdaptor.Hdf5Frame;
...
public class AmuseWindow extends CommonWindow {
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        if (timer.isInitialized()) {
            ...
            displayContext(currentFrame, starFBO, starHaloFBO, gasFBO, hudFBO, axesFBO);
    ...
    private synchronized void displayContext(Hdf5Frame frame, FBO starFBO, FBO starHaloFBO, FBO gasFBO, FBO hudFBO,
            FBO axesFBO) {
        ...
        if (settings.getStereo()) {
            ...
            renderScene(gl, mv, frame, starHaloFBO, starFBO, gasFBO, axesFBO);
            ...
            renderScene(gl, mv2, frame, starHaloFBO, starFBO, gasFBO, axesFBO);
    ...
    private void renderScene(GL3 gl, MatF4 mv, Hdf5Frame frame, FBO starHaloFBO, FBO starFBO, FBO gasFBO, FBO axesFBO) {
        ...
        try {
            renderGas(gl, mv, gasFBO, frame);
            renderStars(gl, mv, starFBO, frame);
            renderStarHalos(gl, mv, starHaloFBO, frame);
    ...
    private void renderGas(GL3 gl, MatF4 mv, FBO gasFBO, Hdf5Frame frame) throws UninitializedException {
        if (post_process) {
            gasFBO.bind(gl);
    ...
    private void renderStars(GL3 gl, MatF4 mv, FBO starsFBO, Hdf5Frame frame) throws UninitializedException {
        if (post_process) {
            starsFBO.bind(gl);
            gl.glClear(GL.GL_DEPTH_BUFFER_BIT | GL.GL_COLOR_BUFFER_BIT);
        }
        ...
        frame.drawStars(gl, animatedTurbulenceShader, mv);
    ...
    private void renderStarHalos(GL3 gl, MatF4 mv, FBO starHaloFBO, Hdf5Frame frame) throws UninitializedException {
        if (post_process) {
            starHaloFBO.bind(gl);
            gl.glClear(GL.GL_DEPTH_BUFFER_BIT | GL.GL_COLOR_BUFFER_BIT);
        }
        ...
        frame.drawStars(gl, pplShader, mv);
    ...
    public void makeSnapshot(String fileName) {
        ...
        displayContext(currentFrame, starFBO4k, starHaloFBO4k, gasFBO4k, hudFBO4k, axesFBO);
