...
public class DistanceComparator implements Comparator<StarModel> {
    ...
    public int compare(StarModel m1, StarModel m2) {
        ...
        if (VectorFMath.length(m1Location.sub(cameraPosition)) > VectorFMath.length(m2Location.sub(cameraPosition))) {
            return -1;
        } else if (VectorFMath.length(m1Location.sub(cameraPosition)) < VectorFMath.length(m2Location.sub(cameraPosition))) {
            return 1;
        } else {
            return 0;
        }
