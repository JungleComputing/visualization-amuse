...
public class VBO {
    ...
    protected IntBuffer  vboPointer;

    /** The internal OpenGL pointer to the Array Buffer */
    protected IntBuffer  bufferPointer;
    ...
    public VBO(GL3 gl, GLSLAttrib... attribs) {
        ...
        for (final GLSLAttrib attrib : attribs) {
            System.out.println("attrib capacity " + attrib.buffer.capacity());
            System.out.println("attrib size " + Buffers.SIZEOF_FLOAT);
            size += ((FloatBuffer) attrib.buffer).capacity() * Buffers.SIZEOF_FLOAT;
        }

        gl.glBufferData(GL3.GL_ARRAY_BUFFER, size, (Buffer) null,
                GL3.GL_STATIC_DRAW);

        System.out.println("total buffer size " + size);

        // Copy the GLSL Attribute data into the internal OpenGL buffer
        int offset = 0;
        for (final GLSLAttrib attrib : attribs) {
            int copySize = ((FloatBuffer) attrib.buffer).capacity() * Buffers.SIZEOF_FLOAT;

            System.out.println("offset " + offset);
            System.out.println("copySize " + copySize);

            gl.glBufferSubData(GL3.GL_ARRAY_BUFFER, offset, copySize,
                    attrib.buffer);
            offset += copySize;
        }
